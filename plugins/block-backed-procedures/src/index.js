...
/**
 * Mixin for all of the methods that relate to / return information about
...
const procedureDefGetDefMixin = function() {
  const mixin = {
     ...
     * @return {!Blockly.procedures.IProcedureModel} The data model for this
     *     procedure block.
     */
    getProcedureModel() {
      return this.model_;
     ...
     * @return {!Array<string>} List of variable Blockly.Names.
     */
    getVars: function() {
  ...
  mixin.model_ = new Blockly.procedures.ObservableProcedureModel(
      this.workspace,
...
Blockly.Extensions.register('procedure_def_get_def_mixin', procedureDefGetDefMixin);
...
const procedureDefVarMixin = function() {
  const mixin = {
    ...
    updateVarName: function(variable) {
      ...
      if (containsVar) {
        Blockly.procedures.triggerProceduresUpdate(this.workspace);
      }
    },
  };
...
const procedureDefUpdateShapeMixin = {
  ...
  updateParameters_: function() {
    ...
    // The field is deterministic based on other Blockly.Events, no need to fire.
...
const procedureDefMutator = {
  ...
  loadExtraState: function(state) {
    ...
    if (state['params']) {
      for (let i = 0; i < state['params'].length; i++) {
        ...
        this.getProcedureModel().insertParameter(
            new Blockly.procedures.ObservableParameterModel(this.workspace, name, paramId, id), i);
      }
    }
  ...
  noBlockHasClaimedModel_(procedureId) {
    ...
    return this.workspace.getAllBlocks(false).every(
        (b) => !Blockly.procedures.isProcedureBlock(b) || !b.isProcedureDef() ||
...
const validateProcedureParamMixin = {
   ...
   * Beyond this, all Blockly.Names are legal.
  ...
  validator_: function(varName) {
    ...
    // Prevents duplicate parameter Blockly.Names in functions
    const workspace =
...
const procedureCallerGetDefMixin = function() {
  const mixin = {
    ...
    /**
     * @return {Blockly.procedures.IProcedureModel} The procedure model associated with this
     *     block.
     */
    getProcedureModel() {
      return this.model_;
     ...
     * @param {Array<string>} params The param Blockly.Names of the procedure model
     *     to find.
     * @return {Blockly.procedures.IProcedureModel} The procedure model that was found.
     */
    ...
    /**
     * @return {Blockly.Workspace} The main workspace (i.e. not the flyout workspace)
...
Blockly.Extensions.registerMutator('procedure_caller_mutator', procedureCallerMutator);
...
const procedureCallerOnChangeMixin = {
   ...
   * block.
   * @param {Blockly.Block} defBlock The definition block to check against.
   ...
   * @param {Array<string>} params The Blockly.Names of the parameters to create.
   * @return {Blockly.procedures.IProcedureModel} The procedure model associated with the new
...
const procedureIfReturnOnChangeMixin = {
   ...
   * @param {!Blockly.Events.Abstract} e Move event.
   * @this {Blockly.Block}
   */
  onchange: function(e) {
